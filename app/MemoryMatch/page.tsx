...
const MemoryMatch = () => {
  ...
  const handleSquareClick = (square: Square) => {
    if (!square.isFlipped && !square.isMatched && flippedSquares.length < 2) {
      const updatedSquares = squares.map(s => (s.id === square.id ? { ...s, isFlipped: true } : s))
  ...
  const checkMatch = (flipped: Square[]) => {
    ...
    if (first.color === second.color) {
      ...
      setTimeout(() => {
        setSquares(prevSquares =>
          prevSquares.map(s => (flipped.some(f => f.id === s.id) ? { ...s, isFlipped: false } : s)),
        )
      }, 1000)
      ...
      {!gameStarted ? (
        <StyledContainerButton>
          <StyledButtonFirst
            ...
            onClick={() => clickButton('first', 12)}
          >
            12 Squares
          </StyledButtonFirst>
          <StyledButtonSecond
            $isActive={activeButton === 'second'}
            onClick={() => clickButton('second', 16)}
          >
            16 Squares
          </StyledButtonSecond>
